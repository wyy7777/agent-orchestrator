                    ...
                    try:
                        ...
                        for step_exec, step_def in group:
                            if step_exec.status != StepStatus.COMPLETED.value:
                                ...
                                step_exec.completed_at = datetime.now(timezone.utc)
                        task.status = TaskStatus.FAILED.value
                        task.error_message = "步骤组超时"
                        task.completed_at = datetime.now(timezone.utc)
                        await db.commit()
                        failed_names = [sd.name for se, sd in group if se.status == StepStatus.FAILED.value]
                        logger.error(f"步骤组超时: {failed_names}")
                        for name in failed_names:
                            await self._emit("step_failed", task_id, name, "超时")
                        await self._emit("task_failed", task_id, workflow_def.name, "步骤组超时")
                        return
                    except Exception as e:
                        for step_exec, step_def in group:
                            if step_exec.status != StepStatus.COMPLETED.value:
                        ...
                        for name in failed_names:
                            await self._emit("step_failed", task_id, name, str(e))
                        await self._emit("task_failed", task_id, workflow_def.name, str(e))
                        return
